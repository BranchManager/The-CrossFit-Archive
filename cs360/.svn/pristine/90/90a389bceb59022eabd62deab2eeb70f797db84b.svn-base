...
process_files(char * tfile,JRB hardlinks,JRB rp)
{
	...
	int length;
	length = strlen(tfile);
	type= fwrite(&length, 4,1, stdout); //prints the length of the file
	type= printf("%s",tfile);
	fwrite(&buffer,sizeof(struct stat),1,stdout);
	//filename and struct stat then data of file
	...
	{
		fprintf(stderr,"weird directory\n");
...
void REA(JRB hardlinks,JRB direct)
{
	...
	//helps update the file times 

	while((k =fread(&LENGTH,4,1,stdin))== 1)
	{
		fread(FILENAME,1,LENGTH,stdin);
